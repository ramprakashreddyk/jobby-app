...
const Sidebar = () => {
    ...
    const requiredUser: any = JSON.parse(localStorage.getItem("currentUser") as string)
        ...
        <ProtectedRoute>
            <div className='flex bg-black min-h-screen p-5'>
                ...
                <div className='flex flex-col gap-4 items-center justify-start ml-[25%] mt-16'>
                    ...
                    {filter?.length > 0 ? (
                        filter?.map((each: any) => (
                            <Link href={`/jobs/${each.id}`} className="cursor-pointer bg-gray-800 p-4 rounded-md text-gray-400 mt-16">
                        ...
                        data?.jobs?.map((each: any) => (
                            <Link href={`/jobs/${each.id}`} className="bg-gray-800 p-4 rounded-md text-gray-400 mt-16">
                                <div className="flex items-center">
                                    <img src={each.company_logo_url} alt="company_logo" className="w-10 h-10" />
                                    <div className="ml-4">
                                        <h1 className="text-white text-lg">{each.title}</h1>
